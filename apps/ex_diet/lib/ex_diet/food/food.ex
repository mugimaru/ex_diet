defmodule ExDiet.Food do
  ...
  def change_recipe(%Recipe{} = recipe, params \\ %{}) do
    Recipe.changeset(recipe, params)
  end
  ...
  defp count_references(%Ingredient{} = entity),
    do: count_references(entity, [:meals, :recipe_ingredients])
  ...
  defp broadcast_change({:ok, struct}, event) do
    Phoenix.PubSub.broadcast(
      ExDiet.PubSub,
      topic_for_struct(struct.__struct__),
      {__MODULE__, event, struct}
    )
