<template>
  <div>
    <b-card no-body :border-variant="today ? 'info' : 'default'" class="calendar-widget">
      <div slot="header">
      ...
      <table class="table table-bordered widget-table" v-if="hasAnyData && !chartMode">
        <thead>
          <th>Meal</th>
          <th>Weight</th>
          <th class="d-none d-lg-table-cell">Protein</th>
          <th class="d-none d-lg-table-cell">Fat</th>
          <th class="d-none d-lg-table-cell">Carbonhydrate</th>
          <th>Energy</th>
          <template v-if="editCalendar">
            <th class="d-block d-lg-none">
              <b-dropdown variant="outline-primary" block size="sm" right>
        ...
        <draggable v-if="editCalendar" v-model="editCalendar.meals" :tag="'tbody'">
          <tr v-for="(meal, i) in editCalendar.meals" :key="meal.id || meal.tmpId">
            ...
            <td class="d-none d-lg-table-cell">{{ mealsNutritionFacts[i].protein | toFixed(2) }}</td>
            <td class="d-none d-lg-table-cell">{{ mealsNutritionFacts[i].fat | toFixed(2) }}</td>
            <td class="d-none d-lg-table-cell">{{ mealsNutritionFacts[i].carbonhydrate | toFixed(2) }}</td>
        ...
        <tfoot>
          <th colspan="2"></th>
          <th class="d-none d-lg-table-cell">{{ totalNutritionFacts.protein | toFixed(2) }}</th>
          <th class="d-none d-lg-table-cell">{{ totalNutritionFacts.fat | toFixed(2) }}</th>
          <th class="d-none d-lg-table-cell">{{ totalNutritionFacts.carbonhydrate | toFixed(2) }}</th>
...
export default {
  ...
  computed: {
    ...
    hasAnyData() {
      return (
        (this.editCalendar && this.editCalendar.meals.length > 0) || (this.calendar && this.calendar.meals.length > 0)
      );
    },
